...
import {
  TokamakEntryPoint,
  TokamakEntryPoint__factory,
  IERC20,
  IEntryPoint,
  TokamakAccount,
  TokamakAccount__factory,
  TokamakAccountFactory__factory,
...
export async function calcGasUsage (rcpt: ContractReceipt, entryPoint: TokamakEntryPoint, beneficiaryAddress?: string): Promise<{ actualGasCost: BigNumberish }> {
...
export function getAccountInitCode (owner: string, factory: TokamakAccountFactory, salt = 0): BytesLike {
  return hexConcat([
    factory.address,
...
export async function getAccountAddress (owner: string, factory: TokamakAccountFactory, salt = 0): Promise<string> {
...
export async function createAccount (
  ...
  accountOwner: string,
  entryPoint: string,
  _factory?: TokamakAccountFactory
):
  Promise<{
    proxy: TokamakAccount
    accountFactory: TokamakAccountFactory
    implementation: string
  }> {
  let accountFactory = _factory
  if (!accountFactory) {
    const accountImplementation = await new TokamakAccount__factory(ethersSigner)
    accountFactory = await new TokamakAccountFactory__factory(ethersSigner).deploy(entryPoint, accountImplementation.address)
  }
  // const accountFactory = _factory ?? await new TokamakAccountFactory__factory(ethersSigner)
  //   .deploy(entryPoint)
  const implementation = await accountFactory.accountImplementation()
  await accountFactory.createAccount(accountOwner, 0)
  const accountAddress = await accountFactory.getAddress(accountOwner, 0)
  const proxy = TokamakAccount__factory.connect(accountAddress, ethersSigner)
  return {
    implementation,
    accountFactory,
    proxy
...
export function parseValidationResult (messageArgs: string ): { returnInfo: ReturnInfo,  senderInfo:StakeInfo, factoryInfo:StakeInfo, paymasterInfo:StakeInfo } {
    let args = messageArgs.substring(messageArgs.indexOf("errorArgs=")+10, messageArgs.indexOf(", errorName="))
    let argsJson = JSON.parse(args)
    let returnInfo = parseReturnInfo(argsJson[0])
    let senderInfo = parseStakeInfo(argsJson[1])
    let factoryInfo = parseStakeInfo(argsJson[2])
    let paymasterInfo = parseStakeInfo(argsJson[3])

   return { returnInfo, senderInfo, factoryInfo, paymasterInfo}
}

function parseReturnInfo(inputStr: Array<any>) {
   return {
      preOpGas: ethers.BigNumber.from(inputStr[0]),
      prefund:ethers.BigNumber.from(inputStr[1]),
      sigFailed: inputStr[2],
      validAfter: inputStr[3],
      validUntil: inputStr[4],
      paymasterContext: inputStr[5]
  } as ReturnInfo
}


function parseStakeInfo(inputStr: Array<any>) {
  return {
    stake: ethers.BigNumber.from(inputStr[0]),
    unstakeDelaySec: ethers.BigNumber.from(inputStr[1])
 } as StakeInfo
}
